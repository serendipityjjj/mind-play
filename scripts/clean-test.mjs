...
async function cleanTestRoom() {
  ...
  // Apply virtual reset for all activities in TEST room
  await supabase.from("mindplay_live_entries").upsert({
    room_code: "TEST",
    activity: "radio_stories__teacher",
    entry_key: "state",
    student_id: "teacher",
  ...
  await supabase.from("mindplay_live_entries").upsert({
    room_code: "TEST",
    activity: "johari_control__teacher",
    entry_key: "state",
    student_id: "teacher",
    student_name: "Teacher",
  ...
  await supabase.from("mindplay_live_entries").upsert({
    ...
    student_id: "teacher",
    student_name: "Teacher",
    payload: { reset_at: nowIso }
  }, { onConflict: "room_code,activity,entry_key" });

  console.log("✅ Successfully cleaned and virtually reset all TEST room rows from DB.");
}
