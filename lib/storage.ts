...
export const loginUser = async (studentId: string, name: string): Promise<UserProfile | null> => {
  const users = getStoredUsers();
  ...
  // 서버 프로필 조회
  let serverName = name;
  let serverNickname = `${name || "마음"}구름`;
  try {
    const serverRes = await fetchStudentProfileFromServer(studentId);
    if (serverRes?.profile?.studentName) {
      serverName = serverRes.profile.studentName;
    }
    if (serverRes?.profile?.nickname) {
      serverNickname = serverRes.profile.nickname;
    }
  } catch (err) {
    console.error("Failed to fetch server profile during loginUser:", err);
  }
