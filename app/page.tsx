...
function App() {
  ...
  const [currentLesson, setCurrentLesson] = useState(1);
        ...
        const pullServerDatabase = async () => {
          try {
            ...
            if (db && isMounted) {
              ...
              // 2. 1단계 학생 실시간 사연 동기화 (Room stories)
              if (Array.isArray(db.stories)) {
                setStudentRealWorries(db.stories.map((s: any) => ({
                  id: s.id,
                  studentId: s.studentId,
                  author: "익명 친구",
                  category: s.category || "또래/교우",
                  content: s.content || s.text,
                  text: s.content || s.text,
                  likes: s.likes || 0,
                  submittedAt: s.submittedAt || s.createdAt
                })));
              } else if (Array.isArray(db.studentRealWorries)) {
                setStudentRealWorries(db.studentRealWorries);
              }

              // 3. 라디오 하트 동기화
              if (db.radioHearts || db.hearts) {
                const h = db.radioHearts || db.hearts;
                setRadioHearts(h);
                try { localStorage.setItem("mindplay_radio_hearts", JSON.stringify(h)); } catch(e){}
              }
      ...
      const handleAddCustomWorry = async (e: React.FormEvent) => {
        ...
        const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
        const entryKey = `${studentId}_${Date.now()}`;
        const worryPayload = {
          studentId,
        ...
        const res = await saveLiveEntry(cleanRoom, "radio_stories", entryKey, worryPayload, studentId, studentName);
        if (res.success) {
          setCustomWorryText("");
          triggerConfetti();
