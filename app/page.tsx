...
function App() {
  ...
  const [currentLesson, setCurrentLesson] = useState(1);
      ...
      // 방 코드 관리 (기본값 CLASS1, 교사/학생 입력 가능)
      ...
      useEffect(() => {
        ...
        const pollTimer = setInterval(pullServerDatabase, 10000);
      ...
      // 서버에서 내게 도착한 친구의 실제 선물 강점 5개 목록
      ...
      const handleAddCustomWorry = (e) => {
        ...
        const newWorry = {
          ...
          text: customWorryText.trim(),
          createdAt: new Date().toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })
        };
        // Optimistic UI update
        const localObj = {
          id: `story_temp_${Date.now()}`,
          studentId: studentId,
          author: "익명 친구",
          category: customWorryCategory,
          content: customWorryText.trim(),
          likes: 0,
          submittedAt: newWorry.createdAt
        };
        setStudentRealWorries(prev => [localObj, ...prev.filter(w => w.studentId !== studentId)]);
        syncWithServerDb("SUBMIT_STORY", newWorry);
        setCustomWorryText("");
        triggerConfetti();
      ...
      const handleTeacherDispatchWorries = async () => {
        ...
        if (pw === "8888") {
          const allStudentIds = CLASS_STUDENTS.map(s => s.studentId);
          await syncWithServerDb("DISPATCH_ASSIGN", { allStudentIds });
          setIsWorryDispatched(true);
      ...
      const handleResetWorrySession = async () => {
        if (confirm("정말로 모든 사연과 배정 상태를 초기화하시겠습니까? (해당 수업방 초기화)")) {
          ...
          await syncWithServerDb("RESET_STORIES", {});
                            ...
                            <div className="flex flex-wrap items-center gap-2.5">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  const targetId = partnerInfo.targetPartner?.id || myJohariTargetPartnerId;
                                  try {
                                    const payload = {
