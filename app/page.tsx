              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-teal-700 via-emerald-700 to-indigo-800 text-white p-6 sm:p-8 rounded-3xl shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 no-print self-start md:self-auto">
                    <button
                      type="button"
                      onClick={() => {
                        const nextState = !isEssayUnlocked;
                        setIsEssayUnlocked(nextState);
                        try {
                          localStorage.setItem("mindplay_essay_unlocked", nextState ? "true" : "false");
                        } catch(e){}
                        triggerConfetti();
                        alert(nextState
                          ? "🔓 [마음에세이 개방 완료] 이제 모든 학생들이 상단 [📖 마음에세이] 탭에 접속하여 챗봇 대화 기반 에세이를 작성할 수 있습니다!"
                          : "🔒 [마음에세이 잠금 완료] 학생들의 [📖 마음에세이] 탭 접근이 제한되었습니다."
                        );
                      }}
                      className={`px-4 py-2 rounded-xl text-xs font-dodum font-bold shadow flex items-center gap-1.5 transition active:scale-95 cursor-pointer ${
                        isEssayUnlocked
                      ...
                      title="클릭하여 학생들의 마음에세이 탭 접속을 열거나 닫을 수 있습니다"
                    >
                      <span>{isEssayUnlocked ? "🔓 마음에세이 열림" : "🔒 마음에세이 닫힘"}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const next = !isJohariPartnerAssigned;
                        setIsJohariPartnerAssigned(next);
                        try { localStorage.setItem("mindplay_johari_partner_assigned", next ? "true" : "false"); } catch(e){}
                        triggerConfetti();
                        alert(next
                          ? "🤝 [2단계 친구 배정 완료] 1학년 3반 26명의 1:1 순환 매칭이 완료되어 학생들의 Step B(친구 강점 발견)가 개방되었습니다!"
                          : "🔒 [2단계 친구 배정 취소] 학생들의 Step B 접근이 대기 상태로 전환되었습니다."
                        );
                      }}
                      className={`px-4 py-2 rounded-xl text-xs font-dodum font-bold shadow flex items-center gap-1.5 transition active:scale-95 cursor-pointer ${
                        isJohariPartnerAssigned ? "bg-teal-600 text-white ring-2 ring-teal-300" : "bg-rose-600 text-white ring-2 ring-rose-300"
                      }`}
                      title="클릭하여 2단계 짝꿍 친구 배정을 열거나 닫을 수 있습니다"
                    >
                      <span>{isJohariPartnerAssigned ? "🤝 2단계 친구 배정됨" : "👥 2단계 친구 배정하기"}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const next = !isJohariUnlocked;
                        setIsJohariUnlocked(next);
                        try { localStorage.setItem("mindplay_johari_unlocked", next ? "true" : "false"); } catch(e){}
                        triggerConfetti();
                        alert(next
                          ? "📢 [2단계 조하리의 창 결과 전송 완료] 모든 학생의 Step C(4개의 창) 및 Step D(나다움 문장)가 실시간 공개되었습니다!"
                          : "🔒 [2단계 조하리의 창 잠금 완료] 학생들의 Step C/D 결과 확인이 대기 상태로 전환되었습니다."
                        );
                      }}
                      className={`px-4 py-2 rounded-xl text-xs font-dodum font-bold shadow flex items-center gap-1.5 transition active:scale-95 cursor-pointer ${
                        isJohariUnlocked ? "bg-emerald-600 text-white ring-2 ring-emerald-300" : "bg-indigo-600 text-white ring-2 ring-indigo-300"
                      }`}
                      title="클릭하여 2단계 조하리의 창 분석 결과를 학생들에게 전송합니다"
                    >
                      <span>{isJohariUnlocked ? "🪟 2단계 결과 전송됨" : "📤 2단계 결과 전송하기"}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        ...
                        triggerConfetti();
                        alert(next
                          ? "📢 [3단계 밸런스 게임 결과 전송 완료] 모든 학생들에게 1학년 3반 밸런스 게임 통계 및 소울메이트/반전 케미 리포트가 실시간 공개되었습니다!"
                          : "🔒 [3단계 밸런스 게임 결과 잠금] 학생 화면에서 리포트가 대기 상태로 전환되었습니다."
                        );
                      }}
                      className={`px-4 py-2 rounded-xl text-xs font-dodum font-bold shadow flex items-center gap-1.5 transition active:scale-95 cursor-pointer ${
                        isBalanceResultBroadcasted ? "bg-emerald-600 text-white ring-2 ring-emerald-300" : "bg-purple-600 text-white ring-2 ring-purple-300"
                      }`}
                      title="클릭하여 3단계 밸런스 게임 학급 통계 및 케미 리포트를 학생들에게 전송합니다"
                    >
                      <span>{isBalanceResultBroadcasted ? "🎯 3단계 밸런스 결과 전송됨" : "📢 3단계 밸런스 결과 보내기"}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const next = !isLesson3GalleryUnlocked;
                        setIsLesson3GalleryUnlocked(next);
                        try { localStorage.setItem("mindplay_lesson3_gallery_unlocked", next ? "true" : "false"); } catch(e){}
                        triggerConfetti();
                        alert(next
                          ? "🏛️ [3단계 학급 갤러리 개방 완료] 모든 학생들에게 1학년 3반 26명의 브랜딩 카드 전시관이 실시간 공개되었습니다!"
                          : "🔒 [3단계 학급 갤러리 잠금] 학생 화면에서 학급 갤러리가 대기 상태로 전환되었습니다."
                        );
                      }}
                      className={`px-4 py-2 rounded-xl text-xs font-dodum font-bold shadow flex items-center gap-1.5 transition active:scale-95 cursor-pointer ${
                        isLesson3GalleryUnlocked ? "bg-emerald-600 text-white ring-2 ring-emerald-300" : "bg-teal-700 text-white ring-2 ring-teal-300"
                      }`}
                      title="클릭하여 3단계 26명 학급 브랜딩 카드 갤러리를 학생들에게 전송/개방합니다"
                    >
                      <span>{isLesson3GalleryUnlocked ? "🏛️ 3단계 갤러리 개방됨" : "🖼️ 3단계 갤러리 개방하기"}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 items-center">
                  {CURRICULUM.map(c => {
                    ...
                    const submittedCount = (c.no === 1 || c.no === 4) ? 0 : c.no === 2 ? 26 : c.no === 3 ? 25 : c.no === 5 ? 19 : 0;
                    return (
                      <button
                        key={c.no}
                        type="button"
