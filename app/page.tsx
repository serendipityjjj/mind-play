      ...
      useEffect(() => {
        ...
        const pullServerDatabase = async () => {
          try {
            ...
            const res = await fetch(`/api/sync?room=${encodeURIComponent(roomCode)}&studentId=${encodeURIComponent(curId)}`);
            if (!res.ok) return;
            ...
            if (json && json.success && json.data && isMounted) {
              ...
              } else if (db.pairs && curId && db.pairs[curId]) {
                setMyJohariTargetPartnerId(db.pairs[curId]);
              }
              ...
              if (db.myReceivedFeedback) {
                ...
                for (const k of Object.keys(db.feedback)) {
                  const fb = db.feedback[k];
