...
import { saveLiveEntry, loadLiveEntries, subscribeLiveRoom, getTeacherState, setTeacherState } from "@/lib/live";
import { supabase } from "@/lib/supabase";
    ...
    const DEFAULT_CLASS_STUDENT_WORRIES: any[] = [];
...
function App() {
      ...
      // [교사용 시뮬레이션] room_code가 "TEST"일 때만 가짜 학생 28명(99901~99928) 활동 데이터 1초 간격 순차 시뮬레이션
      const [isSimulating, setIsSimulating] = useState(false);
      const handleSimulateFakeStudents = async () => {
        if (isSimulating) return;
        setIsSimulating(true);
        const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
        if (cleanRoom !== "TEST") {
          alert("시뮬레이션은 'TEST' 방에서만 사용 가능합니다.");
          setIsSimulating(false);
          return;
        }

        alert("🤖 [가짜 학생 28명 시뮬레이션 시작]\n99901~99928 학생들이 1초 간격으로 사연 등록 및 강점 선택을 진행합니다. 실시간 증가를 확인하세요!");

        const viaItems = ["창의성", "호기심", "판단력", "학습열의", "통찰력", "용기", "끈기", "진실성", "활력", "사랑", "친절/이타성", "사회적지능", "팀워크", "공정함", "리더십", "용서/자비", "겸손", "신중함", "자기조절", "감사", "희망", "유머", "영성"];
        ...
        for (let i = 1; i <= 28; i++) {
          ...
          // 1. 사연 등록
          await saveLiveEntry(cleanRoom, "radio_stories", stId, {
            text: `${stName}의 고민: 중학교 생활에서 친구들과 더 자연스럽게 어울리고 싶어요!`,
            category: i % 2 === 0 ? "또래/교우" : "성적/학업",
            createdAt: new Date().toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })
          }, stId, stName);

          // 2. Step A 강점 등록
          const mine = [viaItems[(i * 2) % viaItems.length], viaItems[(i * 2 + 1) % viaItems.length], viaItems[(i * 2 + 2) % viaItems.length], viaItems[(i * 2 + 3) % viaItems.length], viaItems[(i * 2 + 4) % viaItems.length]];
          const hope = [viaItems[(i * 2 + 5) % viaItems.length], viaItems[(i * 2 + 6) % viaItems.length]];
          await saveLiveEntry(cleanRoom, "johari_step_a", stId, {
            mine,
            hope,
          ...
          // 3. 주크박스 음악 등록 (짝수 학생)
          if (i % 2 === 0) {
            await saveLiveEntry(cleanRoom, "jukebox_tracks", `j_${stId}`, {
              id: `j_${stId}`,
              author: stName,
              theme: "고민·불안할 때",
              artist: "DAY6 (데이식스)",
              title: "한 페이지가 될 수 있게",
              url: "https://youtu.be/vnS_NewSbU4",
              comment: `${stName}의 추천 힐링곡입니다!`,
              likes: 1,
          ...
          // UI 실시간 리프레시 호출
          pullRadioLiveEntries();
          pullJohariLiveEntries();
        ...
        setIsSimulating(false);
        triggerConfetti();
        alert("🎉 [가짜 학생 28명 시뮬레이션 완료]\n28명의 데이터가 성공적으로 실시간 누적되었습니다!");
      };
      ...
      useEffect(() => {
        ...
        } else if (trimmed.startsWith("999") && trimmed.length === 5) {
          const num = trimmed.slice(3);
          setDetectedStudent({ studentId: trimmed, name: `테스트학생${num}`, isTest: true });
      ...
      const handleLoginSubmit = (e) => {
        ...
        // 2.5 테스트 학생 (99901~99930) 로그인 (비번: 학번 또는 0000)
        if (trimmedId.startsWith("999") && trimmedId.length === 5) {
          if (trimmedPw === trimmedId || trimmedPw === "0000") {
            const num = trimmedId.slice(3);
            const testUser = { studentId: trimmedId, name: `테스트학생${num}`, isTest: true };
            setCurrentUser(testUser);
            setIsTeacherMode(false);
            setCurrentTab("activity");
            try { localStorage.setItem("mindplay_user", JSON.stringify(testUser)); } catch(err){}
            setIsLoginModalOpen(false);
            setLoginInputId("");
            setLoginInputPw("");
            triggerConfetti();
  ...
  const [currentLesson, setCurrentLesson] = useState(1);
      ...
      // 사용자 전환 시(로그인, 교사 전환 등) 로컬 스토리지 + Supabase 클라우드 DB에서 실시간 복원
      useEffect(() => {
        let isMounted = true;
        try {
          ...
          // 🌐 클라우드 Supabase DB에서 영구 저장된 일기 및 완료 상태, 프로필 비동기 복원
          if (studentKey && studentKey !== "guest") {
            fetch(`/api/diary?studentId=${encodeURIComponent(studentKey)}`)
              .then(res => res.json())
              .then(data => {
                if (!isMounted || !data.success) return;
                if (Array.isArray(data.diaries) && data.diaries.length > 0) {
                  const cloudDiaries: Record<number, any> = {};
                  const cloudDone: number[] = [];
                  data.diaries.forEach((d: any) => {
                    const lNo = d.lessonNo || 1;
                    cloudDiaries[lNo] = d;
                    cloudDone.push(lNo);
                  });
                  setDiaries(prev => {
                    const merged = { ...prev, ...cloudDiaries };
                    try { localStorage.setItem("mindplay_diaries_" + studentKey, JSON.stringify(merged)); } catch(e){}
                    return merged;
                  });
                  setCompletedLessons(prev => {
                    const set = new Set([...prev, ...cloudDone]);
                    const mergedDone = Array.from(set);
                    try { localStorage.setItem("mindplay_completed_lessons_" + studentKey, JSON.stringify(mergedDone)); } catch(e){}
                    return mergedDone;
                  });
                }
                if (data.profile) {
                  ...
                  if (p.lesson2) {
                    if (Array.isArray(p.lesson2.mySelfStrengths) && p.lesson2.mySelfStrengths.length > 0) {
                      setMySelfStrengths(p.lesson2.mySelfStrengths);
                    }
                    if (Array.isArray(p.lesson2.myAspirationalStrengths) && p.lesson2.myAspirationalStrengths.length > 0) {
                      setMyAspirationalStrengths(p.lesson2.myAspirationalStrengths);
                    }
                  }
                  if (p.lesson3) {
                    if (p.lesson3.balanceAnswers && typeof p.lesson3.balanceAnswers === "object") {
                      setLesson3BalanceAnswers(p.lesson3.balanceAnswers);
                      try { localStorage.setItem("mindplay_balance_answers_" + studentKey, JSON.stringify(p.lesson3.balanceAnswers)); } catch(e){}
                    }
                    if (p.lesson3.brandingCard) {
                      const bc = p.lesson3.brandingCard;
                      if (bc.nickname) setMaskBioNickname(bc.nickname);
                      if (bc.callMeWhen) setMaskCallMeWhen(bc.callMeWhen);
                      if (bc.selfCheer) setMaskSelfCheer(bc.selfCheer);
                      if (Array.isArray(bc.hashtags) && bc.hashtags.length > 0) setLesson3HashTags(bc.hashtags);
                    }
                    if (p.lesson3.keirsey?.checked && Array.isArray(p.lesson3.keirsey.checked)) {
                      setKeirseyChecked(p.lesson3.keirsey.checked);
                    }
                  }
                }
              })
              .catch(e => console.warn("Supabase diary cloud sync error:", e));
          }
        } catch(e){}
        return () => { isMounted = false; };
      }, [currentUser?.studentId]);

      // 방 코드 관리 (기본값 CLASS1, URL 파라미터 ?room=TEST 또는 로컬스토리지 우선)
      const [roomCode, setRoomCode] = useState(() => {
        try {
          ...
          return localStorage.getItem("mindplay_room_code") || "CLASS1";
        } catch(e) { return "CLASS1"; }
      });
      const [roomPhase, setRoomPhase] = useState<"writing" | "assigned" | "done">("writing");
      const [roomAssignedStory, setRoomAssignedStory] = useState<any>(null);
      const [roomReceivedPostcards, setRoomReceivedPostcards] = useState<any[]>([]);

      // 2단계 조하리의 창 서버 실시간 방 상태
      const [roomJohariPhase, setRoomJohariPhase] = useState<"stepA" | "assigned" | "done">("stepA");
      const [roomStrengths, setRoomStrengths] = useState<Record<string, any>>({});
      const [roomJohariPairs, setRoomJohariPairs] = useState<Record<string, string>>({});
      const [roomJohariFeedback, setRoomJohariFeedback] = useState<Record<string, any>>({});
      const [myJohariTargetPartnerId, setMyJohariTargetPartnerId] = useState<string | null>(null);
      const [myReceivedFeedback, setMyReceivedFeedback] = useState<any>(null);
      const [feedbackReasonInput, setFeedbackReasonInput] = useState<string>("");

      // 실시간 다중 컴퓨터(27대 동시 접속) 중앙 서버 방 동기화 헬퍼
      const syncWithServerDb = async (action: string, payload: any) => {
        try {
          const res = await fetch("/api/sync", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ room: roomCode, action, payload })
          });
          return await res.json();
        } catch(e) {
          console.warn("Room DB Sync failed:", e);
          return null;
        }
      };

      // 27대 기기(교사+학생 26명) Supabase Realtime 웹소켓 실시간 구독 + 10초 안전망 폴링 + visibilitychange
      useEffect(() => {
        let isMounted = true;
        const currentCode = (roomCode || "CLASS1").toUpperCase().trim();

        const pullServerDatabase = async () => {
          try {
            const curId = currentUser?.studentId || "guest";
            const res = await fetch(`/api/sync?room=${encodeURIComponent(currentCode)}&studentId=${encodeURIComponent(curId)}`);
            if (!res.ok) return;
            const json = await res.json();
            const db = json?.data || json?.room;
            if (db && isMounted) {
              // Room phase & 배정 사연 동기화
              if (db.phase) {
                setRoomPhase(db.phase);
                if (db.phase === "assigned") {
                  setIsWorryDispatched(true);
                } else if (db.phase === "writing") {
                  setIsWorryDispatched(false);
                }
              }

              if (db.myAssignedStory) {
                setRoomAssignedStory(db.myAssignedStory);
              } else if (db.phase === "writing") {
                setRoomAssignedStory(null);
              }

              if (Array.isArray(db.myReceivedPostcards)) {
                setRoomReceivedPostcards(db.myReceivedPostcards);
              }

              // 2단계 조하리의 창 서버 데이터 동기화
              if (db.johariPhase) {
                setRoomJohariPhase(db.johariPhase);
                if (db.johariPhase === "assigned") {
                  setIsJohariPartnerAssigned(true);
                } else if (db.johariPhase === "done") {
                  setIsJohariPartnerAssigned(true);
                  setIsJohariUnlocked(true);
                } else if (db.johariPhase === "stepA") {
                  setIsJohariPartnerAssigned(false);
                  setIsJohariUnlocked(false);
                }
              }

              if (db.johariStrengths) {
                setRoomStrengths(db.johariStrengths);
                if (curId && db.johariStrengths[curId]) {
                  const myS = db.johariStrengths[curId];
                  if (Array.isArray(myS.mine) && myS.mine.length === 5) {
                    setMySelfStrengths(myS.mine);
                  }
                  if (Array.isArray(myS.hope) && myS.hope.length === 2) {
                    setMyAspirationalStrengths(myS.hope);
                  }
                }
              } else if (db.strengths) {
                setRoomStrengths(db.strengths);
                if (curId && db.strengths[curId]) {
                  const myS = db.strengths[curId];
                  if (Array.isArray(myS.mine) && myS.mine.length === 5) {
                    setMySelfStrengths(myS.mine);
                  }
                  if (Array.isArray(myS.hope) && myS.hope.length === 2) {
              ...
              if (db.johariPairs) {
                setRoomJohariPairs(db.johariPairs);
              } else if (db.pairs) {
                setRoomJohariPairs(db.pairs);
              }

              if (db.myJohariTargetPartnerId) {
                setMyJohariTargetPartnerId(db.myJohariTargetPartnerId);
              } else {
                const pairsObj = db.johariPairs || db.pairs;
                if (pairsObj && curId && pairsObj[curId]) {
                  setMyJohariTargetPartnerId(pairsObj[curId]);
                }
              }

              if (db.johariFeedback) {
                setRoomJohariFeedback(db.johariFeedback);
              } else if (db.feedback) {
                setRoomJohariFeedback(db.feedback);
              }

              if (db.myReceivedFeedback) {
                setMyReceivedFeedback(db.myReceivedFeedback);
              } else {
                const fbObj = db.johariFeedback || db.feedback;
                if (fbObj) {
                  for (const k of Object.keys(fbObj)) {
                    const fb = fbObj[k];
                    if (fb && fb.targetId === curId) {
                      setMyReceivedFeedback(fb);
                      break;
                    }
                  }
                }
              ...
              // (Stories & hearts are fully managed by pullRadioLiveEntries via mindplay_live_entries)
              ...
              if (curId && curId !== "00000" && curId !== "guest") {
                ...
                // 1) 1단계 고민 라디오 실시간 라이브 엔트리 로드
        const pullRadioLiveEntries = async () => {
          try {
            const curId = currentUser?.studentId || "guest";
            // A. 교사 상태 조회 (reset_at, assignMap, phase)
            const teacherRes = await getTeacherState(currentCode, "radio_stories");
            const teacherState = teacherRes.success ? teacherRes.state : {};
            const resetAt = teacherState.reset_at || null;

            if (teacherState.phase) {
              setRoomPhase(teacherState.phase as any);
              setIsWorryDispatched(teacherState.phase === "assigned");
            } else {
              setRoomPhase("writing");
              setIsWorryDispatched(false);
            }

            // B. 학생 사연 목록 조회 (reset_at 이후 데이터 & 빈 사연 완전 제외)
            const storiesRes = await loadLiveEntries(currentCode, "radio_stories", resetAt);
            if (storiesRes.success && isMounted) {
              const liveStories = storiesRes.entries
                .filter(entry => {
                  const raw = entry.payload?.text || entry.payload?.content || "";
                  return typeof raw === "string" && raw.trim().length > 0;
                })
                .map((entry) => {
                  const content = (entry.payload?.text || entry.payload?.content || "").trim();
                  return {
                    id: entry.entry_key,
                    studentId: entry.student_id || entry.payload?.studentId || "unknown",
                    author: "익명 친구",
                    category: entry.payload?.category || "또래/교우",
                    content,
                    text: content,
                    likes: 0,
                    submittedAt: entry.payload?.createdAt || entry.created_at || "방금 전"
                  };
                });

              // C. 하트 집계 조회
              const heartsRes = await loadLiveEntries(currentCode, "radio_hearts", resetAt);
              const heartCounts: Record<string, number> = {};
              const myLikes: Record<string, boolean> = {};

              (heartsRes.entries || []).forEach((hEntry) => {
                const storyKey = hEntry.payload?.targetStoryKey || hEntry.entry_key.split("_")[0];
                heartCounts[storyKey] = (heartCounts[storyKey] || 0) + 1;
                if (hEntry.student_id === curId) {
                  myLikes[storyKey] = true;
                }
              });

              // 사연에 하트수 및 내가 누른 상태 결합
              const finalStories = liveStories.map(s => ({
                ...s,
                likes: heartCounts[s.id] || 0,
                liked: !!myLikes[s.id]
              }));

              setStudentRealWorries(finalStories);

              // D. 내게 배정된 사연 계산
              if (teacherState.assignMap && curId && teacherState.assignMap[curId]) {
                const targetStudentId = teacherState.assignMap[curId];
                const foundStory = liveStories.find(s => s.studentId === targetStudentId);
                if (foundStory) {
                  setRoomAssignedStory(foundStory);
                }
              } else if (teacherState.phase === "writing") {
                setRoomAssignedStory(null);
              }
            }

            // E. 내게 온 위로 엽서 조회
            const postcardsRes = await loadLiveEntries(currentCode, "radio_postcards", resetAt);
            if (postcardsRes.success && isMounted) {
              const myPostcards = postcardsRes.entries
                .filter(e => e.payload?.toStudentId === curId)
                .map(e => ({
                  fromId: e.student_id,
                  fromName: "익명 친구",
                  postcardData: e.payload?.postcardData || e.payload,
                  sentAt: e.payload?.sentAt || e.created_at
                }));
              setRoomReceivedPostcards(myPostcards);
            }
          } catch (e) {
            console.warn("[pullRadioLiveEntries error]", e);
          }
        };

        // 2) 2단계 조하리의 창 실시간 라이브 엔트리 로드
        const pullJohariLiveEntries = async () => {
          try {
            const curId = currentUser?.studentId || "guest";
            
            // A. 교사 제어 상태 조회 (reset_at, isPartnerAssigned, isUnlocked, pairs)
            const controlRes = await getTeacherState(currentCode, "johari_control");
            const controlState = controlRes.success ? controlRes.state : {};
            const resetAt = controlState.reset_at || null;

            if (typeof controlState.isPartnerAssigned === "boolean") {
              setIsJohariPartnerAssigned(controlState.isPartnerAssigned);
              setRoomJohariPhase(controlState.isPartnerAssigned ? (controlState.isUnlocked ? "done" : "assigned") : "stepA");
            }
            if (typeof controlState.isUnlocked === "boolean") {
              setIsJohariUnlocked(controlState.isUnlocked);
            }
            if (controlState.pairs && typeof controlState.pairs === "object") {
              setRoomJohariPairs(controlState.pairs);
              if (curId && controlState.pairs[curId]) {
                setMyJohariTargetPartnerId(controlState.pairs[curId]);
              }
            }

            // B. Step A: 학생들의 강점 5/2 저장 목록 조회
            const stepARes = await loadLiveEntries(currentCode, "johari_step_a", resetAt);
            if (stepARes.success && isMounted) {
              const strengthsMap: Record<string, any> = {};
              stepARes.entries.forEach(e => {
                if (e.entry_key.startsWith("__")) return;
                strengthsMap[e.student_id || e.entry_key] = {
                  mine: e.payload?.mine || [],
                  hope: e.payload?.hope || [],
                  submittedAt: e.payload?.submittedAt || e.created_at
                };
              });
              setRoomStrengths(strengthsMap);

              // 내 학번의 Step A 데이터 복원
              if (curId && strengthsMap[curId]) {
                const myS = strengthsMap[curId];
                if (Array.isArray(myS.mine) && myS.mine.length > 0) {
                  setMySelfStrengths(myS.mine);
                }
                if (Array.isArray(myS.hope) && myS.hope.length > 0) {
                  setMyAspirationalStrengths(myS.hope);
                }
              }
            }

            // C. Step B: 친구에게 선물한 강점 피드백 조회
            const stepBRes = await loadLiveEntries(currentCode, "johari_step_b", resetAt);
            if (stepBRes.success && isMounted) {
              const fbMap: Record<string, any> = {};
              let myReceived: any = null;

              stepBRes.entries.forEach(e => {
                if (e.entry_key.startsWith("__")) return;
                const obsId = e.student_id || e.payload?.observerId;
                const tgtId = e.payload?.targetId;
                if (obsId) {
                  fbMap[obsId] = e.payload;
                }
                if (tgtId === curId) {
                  myReceived = e.payload;
                }
              });

              setRoomJohariFeedback(fbMap);
              setMyReceivedFeedback(myReceived);

              // 내가 보낸 Step B 데이터 복원
              if (curId && fbMap[curId]) {
                const myGift = fbMap[curId];
                if (Array.isArray(myGift.picks) && myGift.picks.length > 0) {
                  setPartnerGiftStrengths(myGift.picks);
                  setHasSentPartnerGift(true);
                }
              }
            }
          } catch (e) {
            console.warn("[pullJohariLiveEntries error]", e);
          }
        };

        // 1) 초기 데이터 로드 (라이브 엔트리 + 기존 DB)
        pullRadioLiveEntries();
        pullJohariLiveEntries();
        pullServerDatabase();

        // 2) 🚀 Supabase Realtime WebSocket 채널 구독 (mindplay_rooms 실시간 감지)
        const channelName = `room_channel_${currentCode}_${Date.now()}`;
        const channel = supabase
          ...
          .on(
            "postgres_changes",
            {
              ...
              filter: `room_code=eq.${currentCode}`
            },
            (payload) => {
              pullRadioLiveEntries();
              pullJohariLiveEntries();
        ...
        // 3) 10초 안전망 폴링 (웹소켓 연결 지연이나 일시 끊김 대비)
        ...
        // 4) 📱 휴대폰 화면 꺼짐/켜짐 및 탭 전환(visibilitychange) 즉시 동기화
        ...
        return () => {
          ...
          // 5) 컴포넌트 언마운트/페이지 이동 시 중복 구독 방지 해제
          supabase.removeChannel(channel);
          document.removeEventListener("visibilitychange", handleVisibilityChange);
          window.removeEventListener("storage", handleStorageChange);
        };
      }, [currentUser?.studentId, roomCode]);
      ...
      // 서버에서 내게 도착한 친구의 실제 선물 강점 5개 목록 (실제 피드백만 사용, 더미 제거)
      const actualReceivedPicks: string[] = (myReceivedFeedback && Array.isArray(myReceivedFeedback.picks) && myReceivedFeedback.picks.length > 0)
        ? myReceivedFeedback.picks
        : [];

      // 조하리의 창 실시간 교차 분석 계산:
      // 1. 열린 창: 나와 친구 모두 고른 강점
      const openArea = mySelfStrengths.filter(s => actualReceivedPicks.includes(s));
      // 2. 보이지 않는 창: 친구만 골라주고 나는 몰랐던 강점
      const blindArea = actualReceivedPicks.filter(s => !mySelfStrengths.includes(s));
      // 3. 숨겨진 창: 나만 고르고 친구는 모르는 강점
      const hiddenArea = mySelfStrengths.filter(s => !actualReceivedPicks.includes(s));
      ...
      // 3단계 활동별 마이페이지 & 서버 영구 저장 헬퍼
      const handleSaveLesson3Section = async (sectionName: string, payload: any) => {
        try {
          await fetch("/api/diary", {
            ...
            body: JSON.stringify({
              ...
              studentId: currentUser.studentId || studentId,
          ...
          triggerConfetti();
          alert(`💾 [${sectionName} 마이페이지 영구 저장 완료!]\n학생 계정 마이페이지에 안전하게 기록되었습니다. ✨`);
        } catch(e) {
          console.error(e);
      ...
      // 4단계 활동별 마이페이지 & 서버 영구 저장 헬퍼
      const handleSaveLesson4Section = async (sectionName: string, payload: any) => {
        try {
          await fetch("/api/diary", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      const handleAddJukebox = async (e) => {
        e.preventDefault();
        ...
        // Save to student profile DB for MyPage display
        try {
          await fetch("/api/diary", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        setJukeboxComment("");
        triggerConfetti();
        alert("🎉 [우리 반 감정 치유 주크박스 & 마이페이지 저장 완료!]\n추천곡이 주크박스와 학생 마이페이지에 영구 저장되었습니다! 🎶");
      };
      ...
      const handleAddCustomWorry = async (e: React.FormEvent) => {
        e.preventDefault();
        const trimmedText = customWorryText.trim();
        if (!trimmedText) {
          alert("고민 사연 내용을 입력해 주세요!");
          return;
        }
        const curStudentId = currentUser?.studentId || studentId;
        const curStudentName = currentUser?.name || studentName;
        if (!curStudentId || curStudentId === "guest" || curStudentId === "00000") {
          alert("로그인 후 학번이 확인되어야 사연을 등록할 수 있습니다.");
          return;
        }

        const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
        const entryKey = `${curStudentId}_${Date.now()}`;
        const worryPayload = {
        ...
        try {
          console.log("[handleAddCustomWorry] Saving to mindplay_live_entries...", { cleanRoom, entryKey, curStudentId });
          const res = await saveLiveEntry(cleanRoom, "radio_stories", entryKey, worryPayload, curStudentId, curStudentName);
          if (res.success) {
            setCustomWorryText("");
            setStudentRealWorries(prev => [
              {
                ...
                text: trimmedText,
                likes: 0,
                submittedAt: worryPayload.createdAt
              },
              ...prev.filter(w => w.id !== entryKey)
            ]);
            triggerConfetti();
            alert("🔒 [100% 안심 익명 보장]\n고민 사연이 완벽한 익명으로 학급 서버에 등록되었습니다!\n다른 학생 및 교사 화면에 실시간으로 반영됩니다 💌");
          } else {
            console.error("[handleAddCustomWorry Error]", res.error);
            alert(`⚠️ [저장 실패]\n서버에 사연을 등록하지 못했습니다: ${res.error || "네트워크 확인 필요"}\n작성하신 내용은 지워지지 않았으니 다시 시도해 주세요.`);
          }
        } catch (err: any) {
          console.error("[handleAddCustomWorry Exception]", err);
          alert(`⚠️ [오류 발생]\n사연 등록 중 문제가 발생했습니다: ${err?.message || err}`);
        }
      };

      const handleTeacherDispatchWorries = async () => {
        const pw = prompt("교사 비밀번호를 입력해주세요:");
        if (pw === "8888") {
          const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
          const teacherStateRes = await getTeacherState(cleanRoom, "radio_stories");
          const resetAt = teacherStateRes.state?.reset_at || null;
          const storiesRes = await loadLiveEntries(cleanRoom, "radio_stories", resetAt);
          const activeStories = storiesRes.entries || [];
          
          let studentIds = Array.from(new Set(activeStories.map(s => s.student_id).filter(Boolean))) as string[];
          if (studentIds.length < 2) {
            studentIds = CLASS_STUDENTS.map(s => s.studentId);
          }

          // 수학적 완전 순열(Perfect Derangement)
          const n = studentIds.length;
          const shift = 1 + Math.floor(Math.random() * (n - 1));
          const assignMap: Record<string, string> = {};
          for (let i = 0; i < n; i++) {
            assignMap[studentIds[i]] = studentIds[(i + shift) % n];
          }

          const saveRes = await setTeacherState(cleanRoom, "radio_stories", {
            phase: "assigned",
            assignMap
          });
          ...
          if (saveRes.success) {
            setIsWorryDispatched(true);
            setRoomPhase("assigned");
            triggerConfetti();
            alert("🎉 [1:1 무작위 배정 완료]\n모든 학생의 사연이 1:1로 겹치지 않게(자기 사연 제외) 무작위 배정되었습니다!\n학생들 화면이 자동으로 배달된 사연 화면으로 전환됩니다.");
          } else {
            alert(`⚠️ 배정 저장 실패: ${saveRes.error}`);
          }
        } else if (pw !== null) {
      ...
      const handleTeacherUnlockJohari = async () => {
        const pw = prompt("교사 비밀번호를 입력해주세요:");
        if (pw === "8888") {
          const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
          await setTeacherState(cleanRoom, "johari_control", {
            isUnlocked: true,
            unlockedAt: new Date().toISOString()
          });
          await syncWithServerDb("UNLOCK_JOHARI_RESULTS", {});
          setIsJohariUnlocked(true);
          setRoomJohariPhase("done");
          triggerConfetti();
          alert("🎉 [2단계 조하리의 창 결과 전송 완료]\n전체 학생의 Step C(4개의 창 분석) 및 Step D(나다움 문장)가 3초 내 자동 개방됩니다!");
        } else if (pw !== null) {
          alert("비밀번호가 일치하지 않습니다.");
        }
      };
      ...
      const handleResetWorrySession = async () => {
        if (confirm("정말로 모든 사연과 배정 상태를 초기화하시겠습니까? (DB 원본은 보존되며 가상 리셋됩니다)")) {
          const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
          ...
          if (saveRes.success) {
            ...
            setRoomPhase("writing");
            setRoomAssignedStory(null);
            setRoomReceivedPostcards([]);
            alert("🧹 해당 수업방의 고민 사연 및 배정 상태가 완전히 초기화되었습니다! (사연 수: 0개)");
          } else {
            alert(`⚠️ 초기화 실패: ${saveRes.error}`);
          }
        }
      };

      const handleToggleLikeWorry = async (storyKey: string) => {
        const curId = currentUser?.studentId || "guest";
        const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
        const entryKey = `${storyKey}_${curId}`;

        const found = studentRealWorries.find(w => w.id === storyKey);
        if (found?.liked) {
          alert("이미 공감 하트를 보냈습니다! ❤️ (1인 1회 공감)");
          return;
        }

        const res = await saveLiveEntry(cleanRoom, "radio_hearts", entryKey, { targetStoryKey: storyKey }, curId, studentName);
        if (res.success) {
          setStudentRealWorries(prev => prev.map(w => w.id === storyKey ? { ...w, likes: w.likes + 1, liked: true } : w));
          triggerConfetti();
        } else {
          alert(`⚠️ 하트 반영 실패: ${res.error}`);
        }
      };

      // 1단계 익명 토닥토닥 마음 우체통: 서버에서 내게 배정해준 사연 조회
      const getAssignedWorryForMe = () => {
        if (roomAssignedStory) {
          return {
            id: roomAssignedStory.id,
            senderTag: "익명 친구의 고민 엽서",
            targetStudentId: roomAssignedStory.studentId,
            category: roomAssignedStory.category || "또래/교우",
            content: roomAssignedStory.text || roomAssignedStory.content,
            isReal: true
          };
        }
        // Fallback if not assigned yet
        return {
          id: "worry_waiting_dispatch",
          senderTag: "익명 친구의 고민 엽서",
          targetStudentId: null,
          category: "또래/교우",
          content: "선생님께서 고민 사연을 배정 중입니다. 잠시만 기다려주세요!",
          isReal: false
        };
      };
      ...
      const hasReceivedComfortLetter = (Array.isArray(roomReceivedPostcards) && roomReceivedPostcards.length > 0) || isTeacherMode;
      ...
      const handleSendComfortLetter = async (e) => {
        e.preventDefault();
        if (!comfortStep1.trim() || !comfortStep2.trim() || !comfortStep3.trim()) {
          alert("3단계 위로 가이드를 모두 성실하게 작성해 주세요! 친구에게 큰 힘이 됩니다 💌");
          return;
        }
        const targetStory = activeAssignedWorry;
        const targetStId = targetStory?.targetStudentId || roomAssignedStory?.studentId;

        await syncWithServerDb("SEND_POSTCARD", {
          storyId: targetStory?.id,
          fromStudentId: studentId,
          toStudentId: targetStId,
          step1: comfortStep1.trim(),
          step2: comfortStep2.trim(),
          step3: comfortStep3.trim(),
      ...
      const handleResetCurrentActivity = (lessonNo) => {
        ...
        if (lessonNo === 1) {
          ...
          const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
          setTeacherState(cleanRoom, "johari_control", {
          ...
          setIsJohariUnlocked(false);
          setIsJohariPartnerAssigned(false);
          setRoomJohariPhase("stepA");
          setRoomStrengths({});
          setRoomJohariPairs({});
          setRoomJohariFeedback({});
          setMyReceivedFeedback(null);
          setLesson2Step("A");
          try {
            localStorage.removeItem("mindplay_johari_unlocked");
            localStorage.removeItem("mindplay_johari_partner_assigned");
            syncWithServerDb("RESET_JOHARI", { roomCode: activeRoomCode });
          } catch(e){}
        } else if (lessonNo === 3) {
          const initAnswers = {};
      ...
      const handleSaveDiary = async () => {
        ...
        let finalImageUrl = currentDiaryImage || "";

        if (currentDiaryImage && currentDiaryImage.startsWith("data:image")) {
          try {
            const upRes = await fetch("/api/upload", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                imageBase64: currentDiaryImage,
                studentId: currentUser.studentId || studentId || "guest",
                lessonNo: targetLessonNo
              })
            });
            ...
            console.warn("Upload drawing error:", err);
          }
        }

        const newDiaryEntry = {
          studentId: currentUser.studentId || studentId || "guest",
          studentName: currentUser.name || studentName || "학생",
          lessonNo: targetLessonNo,
          lessonTitle: curLessonData?.title || `${targetLessonNo}단계 활동 성찰`,
          ...
          diaryText: currentDiaryInput.content || "오늘 하루도 내 감정을 온전히 마주하고 따뜻하게 안아주었습니다.",
        ...
        try {
          ...
          console.error("Server diary save error:", e);
        }
                ...
                <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6 sm:p-8 space-y-6">
                  ...
                  {currentLesson === 1 && (
                    <div className="space-y-6">
                      ...
                      {isComfortModalOpen && (() => {
                        const latestPc = (Array.isArray(roomReceivedPostcards) && roomReceivedPostcards.length > 0) ? roomReceivedPostcards[0] : null;
                        const myOriginalStory = studentRealWorries.find(w => w.studentId === studentId);
                        const displayPostcard = latestPc ? {
                          from: "익명 친구의 따뜻한 답장 💌",
                          matchedWorrySnippet: myOriginalStory ? (myOriginalStory.content || myOriginalStory.text) : "내가 보냈던 고민 사연",
                          step1: latestPc.step1,
                          step2: latestPc.step2,
                          step3: latestPc.step3,
                          stickers: latestPc.stickers && latestPc.stickers.length > 0 ? latestPc.stickers : ["넌 충분히 잘하고 있어! 🌟", "토닥토닥 힘내자 💖"],
                          thanked: false,
                          thankCount: 1
                        } : {
                          from: "익명 친구 🌿",
                        ...
                        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                            ...
                            <div className="p-4 bg-indigo-50 border-2 border-indigo-200 rounded-2xl flex flex-wrap items-center justify-between gap-3 shadow-sm">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  const pw = prompt("교사 비밀번호를 입력해주세요:");
                                  if (pw === "8888") {
                                    const savedStudentIds = Object.keys(roomStrengths);
                                    const totalCount = CLASS_STUDENTS.length;
                                    if (savedStudentIds.length < totalCount) {
                                      const unsubmittedCount = totalCount - savedStudentIds.length;
                                      if (!confirm(`현재 ${unsubmittedCount}명의 학생이 아직 Step A 저장을 완료하지 않았습니다.\n저장 완료된 ${savedStudentIds.length}명의 학생만으로 1:1 무작위 배정을 진행하시겠습니까?`)) {
                                        return;
                                      }
                                    }
                                    const targetIds = savedStudentIds.length >= 2 ? savedStudentIds : CLASS_STUDENTS.map(s => s.studentId);
                                    
                                    // 단방향 순환 순열(Circular Derangement) 생성
                                    ...
                                    await syncWithServerDb("DISPATCH_JOHARI_PAIRS", { studentIds: targetIds });
                                    setIsJohariPartnerAssigned(true);
                                    setRoomJohariPairs(pairsMap);
                                    setRoomJohariPhase("assigned");
                                    triggerConfetti();
                                    alert("🎉 [2단계 학생 1:1 무작위 배정 완료]\n전체 학생의 1:1 파트너가 중복/누락 및 자기 자신 배제(Derangement)로 배정되었습니다!\n학생들의 화면이 Step B로 3초 내 자동 전환됩니다.");
                                  } else if (pw !== null) {
                                    alert("비밀번호가 일치하지 않습니다.");
                                  }
                                }}
                                className={`px-4 py-2 rounded-xl text-xs font-dodum font-bold shadow transition flex items-center gap-1.5 ${roomJohariPhase === "assigned" || isJohariPartnerAssigned ? "bg-indigo-700 text-white" : "bg-indigo-600 hover:bg-indigo-700 text-white hover:scale-105"}`}
                              >
                                <span>🤝</span>
                                <span>{roomJohariPhase === "assigned" || isJohariPartnerAssigned ? "✨ 학생 배정 완료됨 (재배정)" : `🔒 교사 전용: 학생 배정하기 (${Object.keys(roomStrengths).length}/${CLASS_STUDENTS.length}명 저장됨)`}</span>
                              </button>
                            </div>
                          )}
                          ...
                          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-gray-100">
                            ...
                            <div className="flex flex-wrap items-center gap-2.5">
                              {/* 학생용 검사 결과 저장 버튼 (정확히 5/2 충족 시 활성화) */}
                              <button
                                type="button"
                                disabled={mySelfStrengths.length !== 5 || myAspirationalStrengths.length !== 2}
                                onClick={async () => {
                                  if (mySelfStrengths.length !== 5 || myAspirationalStrengths.length !== 2) {
                                    alert("나의 강점 5개와 희망 강점 2개를 모두 선택해야 저장할 수 있습니다. (현재: 나의 강점 " + mySelfStrengths.length + "/5개, 희망 강점 " + myAspirationalStrengths.length + "/2개)");
                                    return;
                                  }
                                  const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
                                  const payload = {
                                    studentId,
                                    name: studentName,
                                    mine: mySelfStrengths,
                                    hope: myAspirationalStrengths,
                                    submittedAt: new Date().toISOString()
                                  };
                                  await saveLiveEntry(cleanRoom, "johari_step_a", studentId, payload, studentId, studentName);
                                  await syncWithServerDb("SAVE_STRENGTHS", payload);
                                  try {
                                    await fetch("/api/diary", {
                                      method: "POST",
                                      headers: { "Content-Type": "application/json" },
                                  ...
                                  triggerConfetti();
                                  alert("💾 [검사 결과 마이페이지 & 서버 영구 저장 완료!]\n나의 강점 5개와 희망 강점 2개가 학급 공용 서버와 마이페이지에 안전하게 등록되었습니다.\n선생님 화면의 [저장 완료 인원]에 즉시 집계되며, 선생님께서 [학생 배정]을 누르면 Step B 화면으로 자동 전환됩니다. 😊");
                                }}
                              ...
                              >
                                <span>💾</span>
                                <span>{mySelfStrengths.length === 5 && myAspirationalStrengths.length === 2 ? "강점 검사 결과 영구 저장하기" : "5/2개 선택 필요 (저장 대기)"}</span>
                              </button>
                      ...
                      {lesson2Step === "B" && (
                        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3">
                            ...
                            {(() => {
                              const partnerId = myJohariTargetPartnerId || (roomJohariPairs && roomJohariPairs[studentId]) || partnerInfo.targetPartner.studentId;
                              const partnerStudent = CLASS_STUDENTS.find(s => s.studentId === partnerId) || { name: partnerInfo.targetPartner.name, studentId: partnerId };
                              return (
                                <span className="text-xs font-dodum font-bold bg-rose-50 text-rose-700 px-3.5 py-1 rounded-full border border-rose-200">
                                  선택된 친구: <strong>{partnerStudent.name}</strong> ({partnerStudent.studentId})
                                </span>
                              );
                            })()}
                          </div>
                          ...
                          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-gray-100">
                            ...
                            <div className="flex flex-wrap items-center gap-2.5">
                              ...
                              {/* 학생용 친구 강점 저장 버튼 (SEND_STRENGTH_FEEDBACK) */}
                              <button
                                type="button"
                                disabled={partnerGiftStrengths.length !== 5}
                                onClick={async () => {
                                  if (partnerGiftStrengths.length !== 5) {
                                    alert("친구에게 선물할 강점 5개를 정확히 선택해주세요!");
                                    return;
                                  }
                                  const targetId = myJohariTargetPartnerId || (roomJohariPairs && roomJohariPairs[studentId]) || partnerInfo.targetPartner?.studentId;
                                  const targetObj = CLASS_STUDENTS.find(s => s.studentId === targetId) || partnerInfo.targetPartner;
                                  try {
                                    const cleanRoom = (roomCode || "CLASS1").toUpperCase().trim();
                                    const entryKey = `${studentId}_${targetId}`;
                                    const payload = {
                                      roomCode: cleanRoom,
                                      observerId: studentId,
                                      observerName: studentName,
                                      targetId: targetId,
                                      targetName: targetObj?.name || "친구",
                                      picks: partnerGiftStrengths,
                                      savedAt: new Date().toISOString()
                                    };
                                    await saveLiveEntry(cleanRoom, "johari_step_b", entryKey, payload, studentId, studentName);
                                    await syncWithServerDb("SEND_STRENGTH_FEEDBACK", payload);
                                    setHasSentPartnerGift(true);
                                    triggerConfetti();
                                    alert(`🎁 [${targetObj?.name || "친구"}에게 선물할 강점 저장 완료!]\n친구를 관찰하고 선택한 강점 5개가 제출되었습니다.\n선생님께서 [결과 보내기]를 완료하면 Step C(4개의 창)가 자동으로 열립니다. 🌸`);
                                  } catch(e) {
                                    console.error(e);
                              ...
                              >
                                <span>🎁</span>
                                <span>친구 강점 제출하기 (5개)</span>
                              </button>

                              {/* Step C 이동 버튼 (결과 공개 시 이동) */}
                              <button
                                type="button"
                                onClick={() => {
                                  setLesson2Step("C");
                                }}
                                className="px-5 py-3 rounded-2xl text-xs sm:text-sm font-dodum font-bold bg-[#2A784B] hover:bg-[#1E5736] text-white shadow transition flex items-center gap-1.5 hover:scale-105"
                              >
                                <span>Step C (4개의 창) 보기</span>
                                <span>➔</span>
                              </button>
                            </div>
                          </div>
                      ...
                      <div className="bg-gradient-to-b from-[#EEF6F0] to-[#E3EFE6] p-6 sm:p-8 rounded-3xl border-2 border-[#1F6B38]/30 shadow-md space-y-6">
                        ...
                        {lesson3Step === 1 && (
                          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-3 border-t border-gray-100">
                              <button
                                type="button"
                                onClick={() => handleSaveLesson3Section("prediction", {
                                  gardnerPredictedTop,
                                  gardnerPredictedHope,
                                  savedAt: new Date().toISOString()
                                })}
                                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-dodum font-bold shadow transition flex items-center gap-1.5"
                              >
                                <span>💾</span>
                                <span>사전 예상 마이페이지 저장</span>
                              </button>

                              <button
                                type="button"
                                onClick={() => {
                                  handleSaveLesson3Section("prediction", {
                                    gardnerPredictedTop,
                                    gardnerPredictedHope,
                                    savedAt: new Date().toISOString()
                                  });
                          ...
                          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-3 border-t border-gray-100">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => handleSaveLesson3Section("gardner", {
                                    answers: gardnerAnswers,
                                    scores: gardnerScores.map(g => ({ name: g.name, score: g.score, id: g.id })),
                                    top1: top1Intel.name,
                                    top2: top2Intel.name,
                                    savedAt: new Date().toISOString()
                                  })}
                                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-dodum font-bold shadow transition flex items-center gap-1.5"
                                >
                                  <span>💾</span>
                                  <span>다중지능 결과 마이페이지 저장</span>
                                </button>
                                ...
                                {gardnerPage < 6 ? (
                                  <button
                                    ...
                                    className="px-5 py-2.5 bg-[#1F6B38] hover:bg-[#144725] text-white rounded-xl text-xs font-dodum font-bold shadow transition hover:scale-105"
                                  >
                                    다음 8문항 ({gardnerPage + 2}/7) ➔
                                  </button>
                                ) : (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      handleSaveLesson3Section("gardner", {
                                        answers: gardnerAnswers,
                                        scores: gardnerScores.map(g => ({ name: g.name, score: g.score, id: g.id })),
                                        top1: top1Intel.name,
                                        top2: top2Intel.name,
                                    ...
                                    className="px-6 py-2.5 bg-gradient-to-r from-emerald-700 to-teal-700 hover:opacity-95 text-white rounded-2xl text-xs sm:text-sm font-dodum font-bold shadow-md transition hover:scale-105"
                                  >
                                    다중지능 완료! 성격 기질로 ➔
                                  </button>
                                )}
                          ...
                          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-3 border-t border-gray-100">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => handleSaveLesson3Section("keirsey", {
                                    checked: keirseyChecked,
                                    counts: keirseyCounts,
                                    primaryCode: primaryTemperamentCode,
                                    primaryTitle: primaryTemperament.title,
                                    savedAt: new Date().toISOString()
                                  })}
                                  className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-dodum font-bold shadow transition flex items-center gap-1.5"
                                >
                                  <span>💾</span>
                                  <span>성격 기질 결과 마이페이지 저장</span>
                                </button>

                                <button
                                  type="button"
                                  onClick={() => {
                                    handleSaveLesson3Section("keirsey", {
                                      checked: keirseyChecked,
                                      counts: keirseyCounts,
                                      primaryCode: primaryTemperamentCode,
                                      primaryTitle: primaryTemperament.title,
                                  ...
                                  className="px-6 py-2.5 bg-[#1F6B38] hover:bg-[#144725] text-white rounded-2xl text-xs sm:text-sm font-dodum font-bold shadow-md transition hover:scale-105"
                                >
                                  종합 분석 & 밸런스 게임으로 ➔
                                </button>
                              </div>
                        ...
                        {lesson3Step === 4 && (
                          <BalanceGameModule
                            ...
                            onAnswerChange={async (qId, val) => {
                              const next = { ...lesson3BalanceAnswers, [qId]: val };
                              setLesson3BalanceAnswers(next);
                              try {
                                localStorage.setItem("mindplay_balance_answers_" + (currentUser?.studentId || "guest"), JSON.stringify(next));
                                await fetch("/api/diary", {
                                  method: "POST",
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({
                          ...
                          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-4 border-t">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => handleSaveLesson3Section("brandingCard", {
                                    nickname: maskBioNickname,
                                    callMeWhen: maskCallMeWhen,
                                    selfCheer: maskSelfCheer,
                                    hashtags: lesson3HashTags,
                                    primaryTemperament: primaryTemperamentCode,
                                    top1Intel: top1Intel.name,
                                    top2Intel: top2Intel.name,
                                    savedAt: new Date().toISOString()
                                  })}
                                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs sm:text-sm font-dodum font-bold shadow transition flex items-center gap-1.5"
                                >
                                  <span>💾</span>
                                  <span>브랜딩 카드 마이페이지 영구 저장</span>
                                </button>

                                <button
                                  type="button"
                                  onClick={() => {
                                    handleSaveLesson3Section("brandingCard", {
                                      nickname: maskBioNickname,
                                      callMeWhen: maskCallMeWhen,
                                      selfCheer: maskSelfCheer,
                                      hashtags: lesson3HashTags,
                            ...
                            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                              ...
                              <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-3 border-t border-gray-100">
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    type="button"
                                    onClick={() => handleSaveLesson4Section("dailyMoments", {
                                      thanksReason,
                                      sorryAdmit,
                                      savedAt: new Date().toISOString()
                                    })}
                                    className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-dodum font-bold shadow transition flex items-center gap-1.5"
                                  >
                                    <span>💾</span>
                                    <span>감정 상황 체크 마이페이지 저장</span>
                                  </button>

                                  <button
                                    type="button"
                                    onClick={() => {
                                      handleSaveLesson4Section("dailyMoments", {
                                        thanksReason,
                                        sorryAdmit,
                                        savedAt: new Date().toISOString()
                                      });
                            ...
                            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 animate-fadeIn">
                              ...
                              <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    if (!lesson4TargetNickname.trim()) {
                                      alert("편지를 전달할 친구의 별명을 적어주세요!");
                                      return;
                                    }
                                    setLesson4Step(4);
                                    setIsDualLetterSaved(true);
                                    handleSaveLesson4Section("dualLetter", {
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3.5">
                      {CURRICULUM.map(c => {
                        ...
                        const isDone = !!d && (!!d.content || !!d.image || !!d.imageUrl);
                        return (
                          <div
                            key={c.no}
                            onClick={() => {
                              ...
                              if (d) {
                                ...
                                if (d.imageUrl || d.image) setCurrentDiaryImage(d.imageUrl || d.image);
                    ...
                    {CURRICULUM.map(c => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {d && (d.imageUrl || d.image) ? (
                            <div className="h-28 w-full rounded-2xl overflow-hidden bg-gray-50 border border-amber-200">
                              <img src={d.imageUrl || d.image} alt="그림일기" className="w-full h-full object-cover" />
                            </div>
                          ) : (
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-teal-700 via-emerald-700 to-indigo-800 text-white p-6 sm:p-8 rounded-3xl shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2 no-print self-start md:self-auto">
                    {/* [교사용 시뮬레이션 버튼 - room_code === 'TEST' 일 때만 표시] */}
                    {(roomCode || "").toUpperCase().trim() === "TEST" && (
                      <button
                        type="button"
                        onClick={handleSimulateFakeStudents}
                        disabled={isSimulating}
                        className="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-bold rounded-xl text-xs shadow-md transition flex items-center gap-1.5 animate-pulse"
                      >
                        <span>🤖</span>
                        <span>{isSimulating ? "28명 시뮬레이션 실행 중..." : "⚡ 가짜 학생 28명 실시간 시뮬레이션"}</span>
                      </button>
                    )}
                ...
                {archiveModal.type === "activity" ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4 font-batang text-sm">
                      {diaries[archiveModal.lessonNo]?.imageUrl || diaries[archiveModal.lessonNo]?.image ? (
                        <div className="w-full rounded-2xl overflow-hidden border-2 border-amber-200 shadow-sm max-h-72 bg-gray-50 flex items-center justify-center">
                          <img
                            src={diaries[archiveModal.lessonNo]?.imageUrl || diaries[archiveModal.lessonNo]?.image}
                            alt="감정 그림일기"
                        ...
                        <div className="p-3.5 bg-white rounded-xl border border-amber-100 text-xs font-batang text-gray-800 leading-relaxed space-y-2">
                          {realDiary ? (
                            <>
                              ...
                              {(realDiary.imageUrl || realDiary.image) && (
                                <div className="max-h-48 rounded-xl overflow-hidden my-2 border border-amber-200">
                                  <img src={realDiary.imageUrl || realDiary.image} alt="그림일기" className="w-full object-cover" />
                                </div>
                              )}
