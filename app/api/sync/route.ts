...
async function getRoomFromSupabase(roomCode: string): Promise<{ ok: boolean; room: any }> {
  ...
  try {
    ...
    if (data?.data) {
      return { ok: true, room: data.data };
    }

    if (!IS_VERCEL) {
      const localDb = loadLocalDatabase();
      if (localDb[code]) {
        return { ok: true, room: localDb[code] };
      }
    }

    return { ok: true, room: null };
  } catch (e) {
    console.error("Supabase getRoom exception:", e);
    return { ok: false, room: null };
  }
}
...
export async function GET(req: NextRequest) {
  ...
  const roomRes = await getRoomFromSupabase(roomCode);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const roomRes = await getRoomFromSupabase(code);
    if (!roomRes.ok) {
      return NextResponse.json(
        { success: false, message: "기존 데이터를 불러오지 못해 저장을 중단했습니다" },
        { status: 503 }
      );
    }

    let roomObj = roomRes.room;
    if (!roomObj) {
